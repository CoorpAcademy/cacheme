...
var HEADER_ETAG_KEY = 'Etag';
var HEADER_LAST_MODIFIED_KEY = 'Last-Modified';
...
function HTTPCache(options) {
    this.options = _.defaults(options || {}, {unit: 'second', noRevalidate: false});
...
HTTPCache.prototype.setHeader = function(res, options) {
    var localOptions = _.defaults(options || {}, this.options);
    res.setHeader(headerKey, this.getHeader(localOptions));
    if (localOptions.noRevalidate === true) {
        this.removeRevalidate(res);
    }
};

HTTPCache.prototype.removeRevalidate = function(res) {
    res.removeHeader(HEADER_ETAG_KEY);
    res.removeHeader(HEADER_LAST_MODIFIED_KEY);
};
